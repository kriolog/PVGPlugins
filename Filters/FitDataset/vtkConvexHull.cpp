...
namespace
{
	...
	int direction( std::pair<double,double> p0,
	...
	void build_hull(std::deque<std::pair<double,double> > &raw_points,
									std::deque<std::pair<double,double> > &output)
	{
	...
	//
		...
		for(std::deque< std::pair<double,double> >::reverse_iterator it = upper_hull.rbegin()+1;
    ...
    RIGHT,
		...
		//----------------------------------------------------------------------
		void getHull(std::deque<Cpoint2D>& inPoints, std::deque<Cpoint2D>& hull)
		{
			std::sort(inPoints.begin(), inPoints.end());
			type = POINT;
			hull.push_back(inPoints.front());

			for(std::deque<Cpoint2D>::iterator it = inPoints.begin()+1;
				it != inPoints.end(); ++it)
			{
				switch(type)
				{
		...
		//----------------------------------------------------------------------
		void mergeLinear(const Cpoint2D& p, std::deque<Cpoint2D>& hull)
		{
			Cpoint2D *pt;
		...
		//----------------------------------------------------------------------
		void mergePlanar(const Cpoint2D& p, std::deque<Cpoint2D>& hull)
		{
			int U=0, i=1, test;
			int n = hull.size();
...
{
	std::deque < std::pair< double,double > > inPoints;
	std::deque < std::pair< double,double > > outPoints;


	for(int i=0; i<ni; i++)
	{
		inPoints.push_back(std::pair<double,double>(input[i][0], input[i][1]));
	}
	...
	int i=0;
	for(std::deque<std::pair<double,double> >::iterator it = outPoints.begin();
		it != outPoints.end(); it++)
	{
...
void vtkConvexHull::incrementalConvexHull2D(double** input, const int ni,
																						double** &output, int& no)
{
	std::deque < Cpoint2D > inPoints;
	std::deque < Cpoint2D > outPoints;
	...
	int i=0;
	for(std::deque<Cpoint2D >::iterator it = outPoints.begin();
		it != outPoints.end(); it++)
	{
		output[i] = new double[2];
		output[i][0] = it->X();
...
//	for(std::list< std::pair<int,double*> >::iterator it = points.begin();
//		it != points.end(); ++it)
//	{
...
//	}
//
//	for(std::list<int*>::iterator it = triangles.begin();
//		it != triangles.end(); ++it)
//	{
